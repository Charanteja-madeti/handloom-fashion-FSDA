...
export default function ProductTrackingPage() {
  ...
  const [adminFilters, setAdminFilters] = useState({ query: '', status: 'all' })
  ...
  function timelineStateForStep(status, step) {
    ...
    const currentIndex = TIMELINE_STEPS.findIndex((timelineStep) => timelineStep.toLowerCase() === normalizedStatus)
    const stepIndex = TIMELINE_STEPS.indexOf(step)
  ...
  async function cancelOrder(order) {
    ...
    if (!canCancelStatuses.includes(currentStatus)) {
      setMessage({ type: 'error', text: `This order cannot be cancelled after status: ${order.tracking_status}` })
      return
    }
        ...
        {isAdmin ? (
          <div className="tracking-filter-bar">
            <input
              ...
              onChange={(event) => setAdminFilters((previous) => ({ ...previous, query: event.target.value }))}
            />
            <select
              className="tracking-input"
              value={adminFilters.status}
              onChange={(event) => setAdminFilters((previous) => ({ ...previous, status: event.target.value }))}
            >
              ...
              return (
                <article key={order.id} className="tracking-order-card">
                  ...
                  <div className="tracking-order-grid">
                    <div>
                      <p><strong>Customer:</strong> {isEditing ? <input className="tracking-input" name="customerName" value={draft.customerName} onChange={handleDraftChange} /> : order.customer_name}</p>
                      <p><strong>Email:</strong> {isEditing ? <input className="tracking-input" type="email" name="customerEmail" value={draft.customerEmail} onChange={handleDraftChange} /> : order.customer_email}</p>
                      <p><strong>Phone:</strong> {isEditing ? <input className="tracking-input" name="phone" value={draft.phone} onChange={handleDraftChange} /> : order.phone}</p>
                      <p><strong>Alternate:</strong> {isEditing ? <input className="tracking-input" name="alternatePhone" value={draft.alternatePhone} onChange={handleDraftChange} /> : order.alternate_phone || '-'}</p>
                    </div>

                    <div>
                      <p><strong>Address:</strong> {isEditing ? <textarea className="tracking-input" name="address" rows={2} value={draft.address} onChange={handleDraftChange} /> : order.address}</p>
                      <p><strong>Nearby:</strong> {isEditing ? <input className="tracking-input" name="nearbyLocation" value={draft.nearbyLocation} onChange={handleDraftChange} /> : order.nearby_location || '-'}</p>
                      <p>
                        <strong>City / District / State:</strong>{' '}
                        {isEditing ? (
                          <span className="tracking-inline-group">
                            <input className="tracking-input" name="city" value={draft.city} onChange={handleDraftChange} />
                            <input className="tracking-input" name="district" value={draft.district} onChange={handleDraftChange} />
                            <input className="tracking-input" name="state" value={draft.state} onChange={handleDraftChange} />
                          </span>
                        ) : (
                          `${order.city}, ${order.district}, ${order.state}`
                        )}
                      </p>
                      <p><strong>Pincode:</strong> {isEditing ? <input className="tracking-input" name="pincode" value={draft.pincode} onChange={handleDraftChange} /> : order.pincode}</p>
                    </div>
                    ...
                    <div>
                      <p>
                        ...
                        {isEditing ? (
                          <select className="tracking-input" name="trackingStatus" value={draft.trackingStatus} onChange={handleDraftChange}>
                            {TRACKING_STATUS_OPTIONS.map((status) => (
                      ...
                      <p><strong>Courier:</strong> {isEditing ? <input className="tracking-input" name="courierName" value={draft.courierName} onChange={handleDraftChange} /> : order.courier_name || '-'}</p>
                      <p><strong>Tracking Number:</strong> {isEditing ? <input className="tracking-input" name="trackingNumber" value={draft.trackingNumber} onChange={handleDraftChange} /> : order.tracking_number || '-'}</p>
                      <p><strong>Current Location:</strong> {isEditing ? <input className="tracking-input" name="currentLocation" value={draft.currentLocation} onChange={handleDraftChange} /> : order.current_location || '-'}</p>
                      <p><strong>Estimated Delivery:</strong> {isEditing ? <input className="tracking-input" type="date" name="estimatedDelivery" value={draft.estimatedDelivery} onChange={handleDraftChange} /> : order.estimated_delivery || '-'}</p>
                      <p><strong>Delivery Notes:</strong> {isEditing ? <textarea className="tracking-input" name="deliveryNotes" rows={2} value={draft.deliveryNotes} onChange={handleDraftChange} /> : order.delivery_notes || '-'}</p>
                    </div>
                  </div>
                  ...
                  {isAdmin ? (
                    <div className="admin-actions">
                      {isEditing ? (
                        <>
                          <button type="button" className="auth-submit" disabled={saving} onClick={() => saveOrder(order.id)}>
                    ...
                    <div className="admin-actions">
                      {['order placed', 'packed'].includes(String(order.tracking_status || '').trim().toLowerCase()) ? (
                        <button type="button" className="link-button" disabled={cancelingOrderId === order.id} onClick={() => cancelOrder(order)}>
