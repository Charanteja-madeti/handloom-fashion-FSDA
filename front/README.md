...
### Vercel backend + Vercel frontend (recommended)

1. Push this repo to GitHub.
2. Create a Vercel project for backend with Root Directory set to `backend`.
3. In backend project environment variables, set:
	- `NODE_ENV=production`
	- `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`
	- `JWT_SECRET`, `JWT_REFRESH_SECRET`
	- `ALLOWED_ORIGINS=https://your-frontend-project.vercel.app`
4. Deploy backend and copy its URL.
5. Create a Vercel project for frontend with Root Directory set to `front`.
6. In frontend project environment variables, set:
